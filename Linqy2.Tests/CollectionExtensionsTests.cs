...
namespace Linqy2.Tests
{
    ...
    public class CollectionExtensionsTests
    {
        ...
        public void Batch_NullCollection_ThrowsArgumentNullException()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => CollectionExtensions.Batch(collection, 5));
        }

        [Test]
        public void Batch_ZeroPartitionSize_ThrowsArgumentOutOfRangeException()
        {
            IEnumerable<int> collection = Enumerable.Range(0, 10);

            Assert.Throws<ArgumentOutOfRangeException>(() => CollectionExtensions.Batch(collection, 0));
        }
        ...
        public void Batch_WithTestCases_ProducesExpectedResults(int partitionSize, string expected)
        {
            IEnumerable<int> collection = Enumerable.Range(1, 10);

            var partitions = CollectionExtensions.Batch(collection, partitionSize);
        ...
        [Test]
        public void PartitionIf_NullCollection_ThrowsArgumentNullException()
        {
            IEnumerable<int> collection = null;
            Func<int, int, bool> groupPredicate = (i1, i2) => i2 == i1 + 1;

            Assert.Throws<ArgumentNullException>(() => CollectionExtensions.PartitionIf(collection, groupPredicate));
        }

        [Test]
        public void PartitionIf_NullGroupPredicate_ThrowsArgumentNullException()
        {
            IEnumerable<int> collection = Enumerable.Range(0, 10);
        ...
        [Test]
        public void PartitionIf_BasicRangeGrouping_ReturnsCorrectGrouping()
        {
            var input = new[]
            {
                1, 2, 3, 5, 6, 7, 9
            ...
            var output = CollectionExtensions.PartitionIf(input, (prev, current) => current == prev + 1).Select(group => $"{group.First()}-{group.Last()}").ToList();
